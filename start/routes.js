...
//lost and found
Route.get("/lostfound/arsip", async ({ response }) => {
  const LostFound = use('App/Models/LostFound')
  const items = await LostFound.query().where('status', 'Selesai').orderBy('created_at','desc').fetch()
...
Route.put("/lostfound/:id", "LostFoundController.putLostFound");
Route.delete("/lostfound/:id", "LostFoundController.deleteLostFound");
...
// Route.post('/lostfound/upload', 'UploadController.store')
